...
export function Habit() {
  ...
  async function fetchHabits() {
    try {
      ...
      console.log(error);
      Alert.alert('Opa', 'Não foi possível buscar os hábitos')      
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleToggleHabits(habitId: string) {
    try {      
      ...
      console.log(error);
      Alert.alert('Opa', 'O status do hábito não foi atualizado')      
    }
  }
        ...
        <View className={clsx("mt-6", {
          ...
          { 
            ...
            dayInfo?.possibleHabits.map(habit => (
              ...
              <CheckBox key={habit.id} title={habit.title} disabled={isDateInPast} checked={completedHabits.includes(habit.id)} onPress={() => handleToggleHabits(habit.id)}/>
